...
## 综述与提案（v1.0 草稿，2026-09-23）

> 体裁：综述 + 提案。前半部分梳理四个领域如何使用"环境"一词，后半部分提出一个统一框架。所有文献均逐篇打开原文核对；引用中的数字均来自原文。实证部分只报告已实测的因子，候选因子在附录中单独标注。
...
本文的主张是：这四种用法可以统一到一个坐标空间。环境是策略之外、能够改变正确解法的一切；它无限维，因此只能投影到有限根过得了检验的轴上；每根轴的准入判据是"改变正确解"，这是因果干预的概念而不是干扰的概念；测量协议是一次一因子的星形设计加两两覆盖数组，报数是按轴的方向导数与翻转率，禁止总分。我们给出五组 22 个因子 75 个取值的第一版，并用 EnvShift 在操作系统轴上的实测（6 个模型在对照格通过率 92% 到 100%，在靶子格降至 5% 到 52%）说明这个框架能测出别的框架测不出的东西。
...
本文分两步填这个空缺。第二到第五章逐一梳理四个社区的用法，每章给出一张覆盖表或对照表，让读者看到空缺具体在哪一格。第六章提出环境坐标空间：定义、轴的准入判据、因子与取值、格的钉住清单、测量协议、报数方式。第七章用已有实测说明它能测出什么。第八章列出尚未解决的问题。
...
按上述检验，第一版共五组 22 个因子 75 个取值。表 D 给出全部，标记含义：* 基准值；✅ 已有任务在该值上通过门一门二；🔧 格已建成但尚无任务；○ 候选，有机制依据但未过四步检验。
...
统计：22 个因子，75 个取值，其中 ✅ 14 个、🔧 5 个、○ 56 个。全因子约 10¹⁰ 量级，只定义不跑。

三条配套规则。**值集合按任务参数化**：运行时栈组的取值相对于任务声明的技术栈解释，"current / old"对 Python 任务是 3.12 对 3.8，对 Node 任务是 22 对 18，对 CUDA 任务是 12 对 11。**基准格按任务族定义**：GPU 任务从 accel=nvidia、toolkit=matched 出发，普通任务从 accel=none 出发；星形设计不变，只是原点不同。**取值集合封闭核心、开放扩展**：一个候选值要有至少一道任务在它上面通过门一门二并带装置读数，才从 ○ 升为核心；新因子同理。"全不全"因此不是一个人拍板的事，而是有任务就进、没任务就在候选区等待。
...
**重复运行是协议的一部分**，不是可选项。第五章的 flaky 研究说明大量表观环境效应是非确定性 [53][54]；我们自己的数据是同一道题在同一台机器上跑五次，约 40% 的格结果不一致。每个格至少重复 n 次，先分离噪声，再归因到轴。
...
**禁止总分。**"Linux 好于 macOS 好于 Windows"这类排序是靶子分布的产物：在我们的数据里，Linux 得分高是因为多数任务的靶子在 macOS 和 Windows 上；在另一个 benchmark 里，Windows 得分最高是因为选了 Windows 应用。总分把靶子分布和策略能力混在一起，没有可比性。这与第四章的共识一致：ImageNet-C、DomainBed、WILDS 都先按轴报 [36][39][40]。
...
第二问不成立是常见情况：大小写敏感族里，Agent 通过双击对话框绕过了文件名查找，机制根本没被触发。这不是环境效应，是干净的负结果，应当如实报告。

---

## 7 实证：这个框架能测出什么

### 7.1 EnvShift 在操作系统轴上的结果

EnvShift 是我们按第六章原则构建的第一个实例，目前只在操作系统轴上完整实例化：96 道任务（Coding 与 GUI）在 Linux、macOS、Windows 三个格上运行，题面一个字节不动。所有任务通过门一（参考解三系统全过）和门二（朴素解只在靶子格挂）。

主要结果：6 个模型在对照格的通过率为 92% 到 100%，在靶子格降至 5% 到 52%。这个差距不是噪声：对照格持平说明模型能力本身没有问题，靶子格的下降来自写死的假设。

已坐实的机制族包括：macOS 独有的文件名 Unicode 归一（NFC/NFD）与 APFS 的 Unicode 9 大小写折叠，对应表 D 的 fs=unicode-normalizing；Windows 的 cp1252 编码族，对应 locale=cp1252；GUI 通道上原生编辑器族的 5 道任务，其中 3 道只有 Linux 能过、2 道只有 macOS 能过，对应 channel=gui 与 os 的交互。这些机制在装置层都有读数，失败签名能指到操作系统规格书的某一行。

负结果同样值得报告。GUI 上的键盘机制族被模型的自适应行为吸收：有提示时 0/30 触发，中性题面 6/30 触发；大小写族被双击对话框绕过。这些是"机制未触发"，按 6.7 的第二问判为干净负结果。

其余 18 个因子中，运行时栈的 14 个格已在 Linux 上按 digest 钉死建成，但尚未配题；权限、区域时间、连通性可在 Linux runner 上实例化（容器按 digest 钉镜像、非 root 与只读挂载、LANG/TZ/libfaketime、--network none），尚未开始。本文不对这些因子报告任何数字。

### 7.2 DSec 旋钮到轴的映射

第三章的表 B 显示基础设施暴露了旋钮但没有语义。表 E 把 DSec 的沙箱参数逐一映射到本文的轴，说明这些轴在生产系统里是真实存在的：
...
**观测通道是轴还是任务的表示。** 终端与 GUI 交付物相同、题面只改一句，看起来是同一任务的两种投影；但通道改变的是策略能看见什么、能做什么，这又符合"策略之外"的定义。本文暂将其作为轴，理由是它通过了四步检验：GUI 上的正确解与终端上的正确解在结构上不同。
...
**Harness 作为环境。** DSec 把 Harness 做成独立的工具包层 [19]，我们的另一项工作把 Harness 作为被测对象。当 Harness 既可以是策略也可以是环境时，第 6.1 的参数化能否覆盖"Harness 与环境的交互"，例如 Harness 的工具结果截断策略与 no-tty 轴的交互，尚未验证。

---

## 9 结论

"环境"这个词被四个社区以四种不兼容的方式使用，而所有用法都绕开了同一个问题：哪些环境因素会改变一道任务的正确解。本文把环境定义为策略之外能改变正确解的一切，把它投影到有限根过得了四步检验的轴上，形成一个笛卡尔积坐标空间；用软件工程的组合测试理论确定测量预算（星形加两两覆盖），用机器学习的按轴报数惯例确定报数方式（方向导数与翻转率，禁止总分），用因果推断的干预概念区分"改变正确解的轴"与"答案不该变的干扰"。第一版空间有五组 22 个因子 75 个取值，其中 14 个取值已有通过门一门二的任务。在操作系统轴上的实测表明，题面一个字节不动，6 个模型的通过率从对照格的 92% 到 100% 降到靶子格的 5% 到 52%，且失败可以归因到具体的写死假设。
...
**本项目内部材料**
[58] EnvShift 技术报告：环境的定义、维度与归因（讨论定稿 2026-09-23）。
[59] EnvShift 环境笛卡尔积 v0.2（2026-09-23）。
...
| 接触面 | 机制 | 装置层读数 | 已坐实取值 |
|---|---|---|---|
| 文件系统 | 大小写敏感 | 创建 a.txt 后读 A.txt 是否成功 | linux 敏感；macos / windows 不敏感 |
| 文件系统 | Unicode 归一 | 以 NFC 名创建，列目录得到的字节 | macos 归一（APFS） |
| 文件系统 | 非法字符、尾点尾空格、路径长度 | 创建含 `:`、尾空格、260+ 字符路径的返回码 | windows 拒绝 |
| 文件系统 | 文件锁、可执行位 | 打开中删除；chmod +x 后能否执行 | windows 锁；windows 无执行位 |
| 进程与 shell | argv 转义 | 含空格与引号的参数回显 | windows（MSYS bash）转义不对称 |
| 进程与 shell | 解释器名、PATH 查找 | `which python` / `python3`；PATHEXT | ubuntu 无 `python`；windows 有 PATHEXT |
| 编码区域时间 | 默认编码、换行 | `open()` 默认编码；`echo` 换行字节 | windows cp1252、CRLF |
| 编码区域时间 | 排序、时钟 | `sort` 与 `ls` 顺序；`date` 输出 | locale 依赖 |
| 桌面惯例 | 编辑器保存内容、新建文档格式、文件名显示 | gsettings / defaults / 注册表导出 | windows-2022 runner HideFileExt=0 |

其余 21 个因子的规格书按同一格式编写：每个取值至少一个机制，每个机制一条装置读数。

## 附录 B：与本文相关的 EnvShift 装置结论

- 同一道题、同一台机器重复五次，约 40% 的格结果不一致：噪声底线，归因前必须重复运行。
- macOS 运行器上的题包大小限制、Windows 上 bash 被系统 WSL 入口截胡、Windows 将以 `/` 开头的环境变量值做 MSYS 路径转换：这些是装置错误，不是环境效应，必须在分诊阶段排除。
- 机制必须有装置层读数，不能从分数反推：Windows 运行器默认显示文件扩展名（HideFileExt=0），一批以"隐藏扩展名"为机制的题因此无效，直到题内恢复出厂默认并声明。
