...
表 C 最后一行是本文的位置。它和其他行的差别集中在"准入判据"一列：只有本文和机器人的动力学随机化以"答案随之改变"准入。
...
**原则一：环境是策略之外的一切。** 被测策略 π 是显式参数。测模型时，运行框架属于环境；测运行框架时，它属于策略。同一个坐标系服务两种测量，只是 π 的边界不同。
...
**Harness 透明性。** Meng 等提出一个问题：harness 满足什么性质，(agent, harness, environment) 的测量才近似 (agent, environment) 的真值。他们指出没有任何评测框架把它当要求 [75]。环境卡片解决的是第三项的声明，透明性要求的是第二项的性质，两者要合起来才够。

**Harness 作为环境。** DSec 把 Harness 做成独立的工具包层 [19]，另一些评测把 Harness 当被测对象。当 Harness 既可以是策略也可以是环境时，第 4.2 节的参数化能否覆盖两者的交互，没有验证。一个例子是工具结果截断策略与 no-tty 轴的交互。
