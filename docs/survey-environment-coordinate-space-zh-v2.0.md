...
## 综述与提案（v2.0 草稿，2026-09-23）

> 体裁：综述 + 提案，三部分。第一部分梳理五个领域如何使用"环境"一词并指出共同的空缺；第二部分给出定义、环境卡片和测量协议，三层可分别接受；第三部分用卡片回填现有 benchmark 与基础设施。所有文献均逐篇打开原文核对；引用中的数字均来自原文。本文不包含作者团队未发表的实验结果。
...
机器人与世界模型社区则把环境当作可随机化或可学习的模拟器。本文的主张是：这些用法可以统一到一个坐标空间。环境是策略之外、能够改变正确解法的一切；它无限维，因此只能投影到有限根过得了检验的轴上；每根轴的准入判据是"改变正确解"，这是因果干预的概念而不是干扰的概念；测量协议是一次一因子的星形设计加两两覆盖数组，报数是按轴的方向导数与翻转率，禁止总分。我们把坐标空间做成一页纸的**环境卡片**：22 个因子各填一个值或"未声明"，加一份钉住清单。我们用它回填了六个现有 benchmark 与平台，展示哪些格能从论文和代码里填出来、哪些填不出来，并说明采用这张卡片不需要新的基建。
...
- **机器人与世界模型** 社区说"在随机化的环境里训练"或"学一个环境的模型"，意思是一个参数可随机化的物理模拟器，或一个从数据学出来的动力学模型。环境是模拟器。

这五种用法各自有效，但放在一起就露出一个空缺：**没有人回答"哪些环境因素会改变一道任务的正确解"**。Benchmark 社区不问，因为环境是常量；基础设施社区不问，因为旋钮只关乎资源与安全；鲁棒性社区问的是反问题，它准入的因子恰恰是答案不该随之变的那些；软件工程社区有方法但没有对象，组合测试的因子是软件自己的配置项，不是 Agent 面对的世界；机器人社区最接近，动力学随机化的参数确实改变正确动作，但它的对象是物理量，不是软件环境。

本文分三部分。第一部分（第二到第六章）逐一梳理五个社区的用法，每章给出一张覆盖表或对照表，让读者看到空缺具体在哪一格。第二部分分三层给出提案，三层可以分别接受：第七章是定义与轴的准入判据；第八章是环境卡片，一页纸的声明模板，坐标空间是它背后的模式；第九章是测量协议与报数方式，只给想测敏感度的人。第三部分（第十章）用卡片回填六个现有 benchmark 与平台，展示哪些坐标能从论文与代码里填出来、哪些填不出来。第十一章列出尚未解决的问题。
...
机器人学习是唯一一个在实践中已经把"改变正确解的环境因子"和"不改变正确解的环境因子"分开处理了十年的领域，只是从未用这两个名字。

**感知侧随机化：答案不变。** 域随机化 [32]、CAD2RL [60]、RCAN [61] 随机化的是纹理、光照、相机位姿、干扰物。目标位置、自由空间、抓取点在所有渲染下都一样，策略要学会对这些因子失明。RCAN 把这一点做到了字面意义上：它训练一个网络把随机化图像映回规范的模拟图像，在策略看到输入之前先把干扰因子去掉。

**动力学侧随机化：答案改变。** Peng 等 [62] 每回合重新采样连杆质量、关节阻尼、摩擦、控制增益、动作时步；OpenAI 的 Dactyl [63] 随机化物体尺寸、质量、摩擦、执行器增益、重力向量；RMA [64] 把环境写成一个 17 维向量（负载质量与位置、电机强度、摩擦、地形高度）。这些参数变了，同一个目标位姿需要不同的力矩，同一个步态需要不同的落脚点。正确动作随环境改变，所以策略的目标与感知侧相反：**不是对因子失明，而是把因子辨识出来**。辨识有显式和隐式两种。显式的如 UP-OSI [65] 和 RMA：一个在线系统辨识网络从最近的状态动作历史回归出环境参数，喂给以环境参数为条件的通用策略；RMA 用 50 步历史回归，部署时辨识模块以 10 Hz 运行，基础策略以 100 Hz 运行，去掉辨识模块后成功率崩溃。隐式的如 Peng 和 Dactyl：策略用 LSTM，让隐状态自己总结动力学；Dactyl 报告与方块交互 5 秒后，LSTM 隐状态能以 80% 的准确率预测方块比平均大还是小。UP-OSI 还给出了本文最想要的图：性能随单个环境参数取值变化的响应曲线。

两篇综述 [66][67] 都按"随机化模拟器的哪个部件"把方法分成视觉随机化和动力学随机化，Muratore 等还指出"有些域参数没有影响，有些是关键的"，建议先辨识关键参数再随机化。但两篇都没有把不变与自适应的二分当作组织原则。按轴报数的 benchmark 已经出现：THE COLOSSEUM [68] 定义了 14 个扰动因子，报告 5 个策略在每个因子上的成功率下降（单因子 30% 到 50%，组合超过 75%），并在真机复刻上按扰动算模拟到真实的相关性；Factor World [69] 用 11 个因子给出按因子的泛化差距排序。只是这两张表把感知侧与动力学侧的因子混在一列里。

对本文的意义有两条。第一，动力学随机化加系统辨识，是"改机制因子"这一概念最成熟的先例，机器人社区用参数取值的响应曲线报敏感度，与第九章的方向导数同构。第二，RMA 式的两阶段设计给 LLM Agent 提示了一种可测的行为：在改机制轴上，正确的 Agent 应当先从"状态动作历史"（命令输出、报错信息、uname、which、locale）探测自己处在哪个格，再决定做法。这正是在线系统辨识的 LLM 版本。第九章的报数可以增加一项：每根轴上有无探测步骤的表现差异，以及探测本身的成本。

**世界模型回答的是另一个问题。** Ha 与 Schmidhuber 的 World Models [70]、DreamerV3 [71]、Genie [72] 与 Genie 3 [73]、UniSim [74] 都把环境定义为一个从数据学出来的观测与动力学模型，用途是替代真实模拟器：在模型里训练策略，再迁回真实环境。它们的评价指标是预测与生成的保真度、一致性时长、动作可控性、内训策略的迁移率。没有一个把命名的环境参数暴露为受控的坐标：Genie 3 的"可提示世界事件"（改天气、加物体）和 UniSim 的文本指令是自由文本的旋钮，不是轴。它们问的是"模拟器够不够真"，不是"哪些因子改变正确解"。在本文的框架里，世界模型是低成本生产格的一种方式，而不是定义格的方式。

# 第二部分 定义、卡片、协议

前面五章的空缺可以归结为一句话：大家都有旋钮，没有人有坐标。第二部分给出一个坐标系，分三层：定义与准入（第七章），环境卡片（第八章），测量协议与报数（第九章）。三层的证据各自独立，可以分别接受。
...
按上述检验，第一版共五组 22 个因子 75 个取值。表 D 给出全部，* 为基准值。每个非基准取值都附有机制依据（第五章的经验 bug 研究、第三章的基础设施参数），但取值是否最终进入核心，取决于是否有任务在它上面通过第 9.1 节的两道门。

**表 D：环境空间 v0.2（环境卡片的坐标部分）**
...
标准要被采用，采用成本必须低于它带来的信息。Model Card 与 Datasheets for Datasets 的经验是：一页纸的填空表，作者花半小时填，审稿人花五分钟查，谁都不必先同意一套理论。环境卡片照此设计。

### 8.1 卡片长什么样

一张环境卡片由四块组成：

**身份**：被描述的对象（benchmark、平台或一次实验）、版本、填写日期、填写人、依据的材料（论文章节、代码路径）。

**坐标**：表 D 的 22 个因子，每个填一个值。允许的值有三类：表 D 中的命名取值；"未声明"，表示材料里找不到；"不适用"，表示该因子对这个对象没有意义（例如纯终端任务的 browser-engine）。每个值旁边写来源：论文第几节，或仓库里哪个文件。**只填能指出来源的值，不填推断的值。**

**钉住清单**：镜像的 digest 或 tag 及其 pin 策略、包版本 pin 策略、运行框架版本、模型版本策略、快照 ID、系统设置导出。填不出来的同样写"未声明"。

**变化与承认**：作者自己变化了哪些因子；作者在局限或讨论中承认了哪些环境敏感性，原句引用。

### 8.2 填写规则

1. 值必须有出处。论文没写、代码里有，标"(代码)"；论文写了，标"(论文 §X)"。
2. "未声明"不是缺点的标记，是信息的标记。一张全是"未声明"的卡片同样有用：它告诉读者这个 benchmark 的结果没有坐标。
3. 卡片描述的是对象**现在**的状态，不是它应该的状态。改进意见不写进卡片。
4. 卡片可以由第三方填写，但要注明；原作者的确认或修正是卡片的最高等级。

### 8.3 卡片与已有定义的关系

环境卡片不推翻已有定义，它给已有定义补坐标。WebArena 的 ⟨S, A, O, T⟩ [7] 是卡片"坐标"块的抽象形式，转移函数 T 里吸收的一切在卡片上被拆成 22 个因子；Epoch 的"动作集合加决定动作效果的上下文" [26] 中的"上下文"就是坐标加钉住清单；Mercor 的"软件、任务、验证器" [27] 中的"软件"是坐标的系统组与运行时栈组。卡片是这些定义的公共细化。
...
机器可读的 JSON schema 与英文模板随本文发布。

## 9 测量协议与报数

第八章的卡片只要求**声明**，本章讨论**测量**：想知道一个策略沿某根轴的敏感度时怎么做。不测敏感度的人可以只用卡片。
...
第六章的系统辨识给报数增加一项：在改机制轴上，策略是否先探测了环境（命令输出、报错、uname、which、locale）再行动，以及探测的成本。这把"写死了假设"和"没有去查"区分开。
...
我们按第八章的规则，为六个对象各填了一张环境卡片：SWE-bench、Terminal-Bench、OSWorld、WebArena、τ-bench 四个 benchmark，以及 DSec 一个训练平台。填写只依据两类材料：论文原文，和官方仓库里的 Dockerfile、配置文件、评测代码。每个值标注来源是论文第几节还是代码哪个文件；两处都找不到的写"未声明"。卡片由本文作者作为第三方填写，尚未经原作者确认。完整的六张卡片随本文发布，本节只给汇总表与读表结论。

三点说明。SWE-bench 的值取自评测框架 v4.1.0 版本（最后一个保留 Dockerfile 生成代码的版本），2023 年的论文描述的是 conda 环境，Docker 框架 2024 年才出现。Terminal-Bench 取 2.0 版任务集与 Harbor 0.23.0 执行器，1.x 版另注。DSec 只有论文没有代码。

### 10.2 汇总表

**表 F：六个对象的坐标（值；来源：代码 / 论文）**
...
| accel | 无（代码） | 每题 gpus 字段，抽样为 0（代码） | 无；KVM 可选（代码） | 未声明 | 未声明 | GPU 容器 / virtio-gpu（论文） |
| toolkit | 未声明 | 未声明 | 未声明 | 未声明 | 未声明 | 未声明 |
| compute-cap | 未声明 | 未声明 | 未声明 | 未声明 | 未声明 | 未声明 |
| gpu-count | 0（代码） | 0，上限 1（代码） | 未声明 | 未声明 | 未声明 | 未声明 |
| os | ubuntu:22.04（代码） | 每题不同：ubuntu:24.04 / python:3.x-slim 等（代码） | Ubuntu 客户机，版本未声明；Windows 10（代码/论文） | 未声明 | 未声明 | 宿主 Linux 7.0，客户机 6.1；Ubuntu 基础镜像；Android VM（论文） |
...
| entry-name | python（代码） | 各题不同；轨迹里是 python3 | 未声明 | python（代码） | python（代码） | 未声明 |
...
| locale | 未声明 | 未声明 | 未声明 | 未声明 | 未声明 | 未声明 |
| tz | Etc/UTC（代码） | 未声明 | 未声明 | 未声明 | 虚构的 2024-05-15 15:00 EST（代码/论文） | 未声明 |
| clock | 未声明 | 未声明 | 未声明 | 未声明 | 冻结 / 虚构（代码） | 未声明 |

### 10.3 读表

**整列空白的因子。** toolkit、compute-cap、locale 六个对象全部未声明。fs 除 DSec 外全部未声明。clock 除 τ-bench 外全部未声明。tz 只有 SWE-bench（代码里 `ENV TZ=Etc/UTC`）和 τ-bench（政策文本里的虚构时间）两处。这三列正是第五章经验 bug 研究指出的跨 OS 失败主因（编码、时区）所在。

**论文不是环境，代码才是。** 大量值只存在于代码里，论文一字未提：SWE-bench 的操作系统（ubuntu:22.04）、运行用户（root）、时区，都只在 Dockerfile 生成代码里；WebArena 的浏览器引擎和 1280×720 视口只在 `envs.py` 里；Terminal-Bench 的 bash 与 tmux 伪终端只在执行器代码里。读论文的人无法知道这些坐标，读代码的人才知道。这意味着两个团队"按论文复现"可以在不同的格里跑。

**钉住清单全是 tag，没有 digest。** SWE-bench 的基础镜像是可变 tag `ubuntu:22.04`，预构建镜像默认拉 `latest`；Terminal-Bench 抽样的 12 个 Dockerfile 没有一个用 `@sha256:`，1.x 版基础镜像带日期 tag 但自己也 `FROM ubuntu:24.04`；WebArena 的站点镜像以日期命名分发 tar 包；DSec 的示例是 `sphinx-9658:official`。按第五章的可复现构建标准，这些格没有一个被 bit-for-bit 钉住。OSWorld 是例外的一半：AWS AMI 按 ID 钉住、VM 快照 `init_state` 每题回滚，但 VM 镜像本身按固定 URL 下载而不校验哈希。

**变化因子的只有一家。** 六个对象里只有 OSWorld 的作者自己变化过卡片上的因子（截图分辨率、观测类型、43 道任务的 OS）。其余五家变化的是 agent、模型、提示词或重复次数。

**承认环境敏感性的只有两家半。** Terminal-Bench 在局限里写了机器资源与容器运行时差异；OSWorld 在未来工作里写了跨硬件软件部署；DSec 说明四种后端"文件系统语义与操作系统能力不同"。SWE-bench、WebArena、τ-bench 没有任何关于硬件或 OS 的表述，SWE-bench 的仓库文档反而写"容器化消除了环境差异"。

**填卡片本身很便宜。** 六张卡片由一个人在一天内从公开材料填出，没有联系任何原作者，没有运行任何代码。这是第八章"采用成本低于信息价值"的直接证据：一个 benchmark 的作者填自己的卡片，只会更快、更准。
...
**被测对象的边界。** 7.1 把策略 π 写成显式参数，但 Harness 与模型的边界在实践中并不清晰：模型的系统提示词属于谁？Harness 的上下文压缩策略属于谁？我们目前的做法是按测量目标划定，测模型时 Harness 钉住，测 Harness 时模型钉住。是否存在一个不依赖测量目标的划分，尚无答案。
...
**交互项的预算。** 两两覆盖数组在 22 个因子 75 个取值上仍然可观。NIST 的交互规则来自传统软件，Agent 的失败是否同样以低阶交互为主，需要实证。如果高阶交互显著，第 9.1 的三层预算要重新设计。

**造题成本。** 每根轴每个取值至少要一道过门一门二的任务，而门一要求参考解在所有相关格上通过。硬件组（GPU、CUDA 版本、算力代际）的任务尤其昂贵。这是这套标准推广的主要瓶颈，也是它需要社区协作的原因。

**Harness 作为环境。** DSec 把 Harness 做成独立的工具包层 [19]，而另一些评测工作把 Harness 本身作为被测对象。当 Harness 既可以是策略也可以是环境时，第 7.1 的参数化能否覆盖"Harness 与环境的交互"，例如 Harness 的工具结果截断策略与 no-tty 轴的交互，尚未验证。

---

## 12 结论

"环境"这个词被四个社区以四种不兼容的方式使用，而所有用法都绕开了同一个问题：哪些环境因素会改变一道任务的正确解。本文把环境定义为策略之外能改变正确解的一切，把它投影到有限根过得了四步检验的轴上，形成一个笛卡尔积坐标空间，并把这个空间做成一页纸的环境卡片；用软件工程的组合测试理论确定测量预算（星形加两两覆盖），用机器学习的按轴报数惯例确定报数方式（方向导数与翻转率，禁止总分），用因果推断的干预概念区分"改变正确解的轴"与"答案不该变的干扰"。第一版空间有五组 22 个因子 75 个取值，每个取值附有来自经验 bug 研究或生产基础设施参数的机制依据，是否进入核心取决于是否有任务通过两道准入门。

这个框架的采用成本很低：基础设施层已经有了全部旋钮，缺的只是给旋钮以语义。我们希望环境卡片成为 Agent 评测报告的一个必填附件：不是"我们在一个可复现的环境里测了"，而是"这是我们的环境卡片，坐标为 e、钉住清单为 P(e)"。想进一步测敏感度的人，再按第九章沿轴报方向导数。
